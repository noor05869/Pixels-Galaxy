...
export type StatusRouteContext = { params: Promise<{ orderNumber: string }> };
...
export function createStatusHandler(
  ...
  return async (request, context) => {
    ...
    if (!ORDER_NUMBER.test(orderNumber)) return jsonResponse({ error: "Invalid order update" }, 400);

    let value: unknown;
    try { value = await request.json(); }
    catch { return jsonResponse({ error: "Invalid order update" }, 400); }
    ...
    try {
      ...
      return jsonResponse({ orderNumber: order.orderNumber, status: order.status, updatedAt: order.updatedAt }, 200);
    } catch (error) {
      if (error instanceof Error && error.message === "Order not found") return jsonResponse({ error: "Order not found" }, 404);
