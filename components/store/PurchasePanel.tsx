...
export function PurchasePanel({ product }: { product: Product }) {
  const [bundle, setBundle] = useState(product.bundles[0]);
  ...
  const add = () => { addItem({ productId: product.id, name: product.name, image: product.media[0].src, unitPrice: bundle.unitPrice, quantity, bundleId: bundle.id, bundleQuantity: bundle.quantity }); setMessage(`${product.name} added to your bag.`); };
  ...
  return <div className="purchase-panel"><p className="kicker">PAKISTAN&apos;S FUN TOY</p><h2>{product.name}</h2><Stars rating={product.rating} count={product.reviews} /><p className="product-description">{product.description}</p>
    ...
    <p className="stock"><span /> IN STOCK — READY TO SHIP</p>
    <fieldset className="bundles"><legend>CHOOSE YOUR ZIPSTRING</legend>{product.bundles.map((item) => <label key={item.id} className={bundle.id === item.id ? "selected" : ""}><input type="radio" name="bundle" checked={bundle.id === item.id} onChange={() => choose(item.id)} /><span><strong>{item.label}</strong>{item.badge && <small>{item.badge}</small>}</span><Price amount={item.unitPrice} compareAt={item.compareAt} /></label>)}</fieldset>
