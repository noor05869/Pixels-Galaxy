...
type CartValue = { lines: CartLine[]; totalQuantity: number; isOpen: boolean; isHydrated: boolean; addItem: (line: CartLine) => void; updateQuantity: (id: string, quantity: number) => void; removeItem: (id: string) => void; clearCart: () => void; openCart: () => void; closeCart: () => void };
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<CartValue>(() => ({ lines, totalQuantity: lines.reduce((sum, line) => sum + line.quantity, 0), isOpen, isHydrated: ready,
    ...
    updateQuantity: (id, quantity) => setLines((current) => current.map((x) => x.productId === id ? { ...x, quantity: clamp(quantity) } : x)), removeItem: (id) => setLines((current) => current.filter((x) => x.productId !== id)), clearCart: () => { setLines([]); try { localStorage.removeItem(key); } catch {} }, openCart: () => setOpen(true), closeCart: () => setOpen(false),
  }), [lines, isOpen, ready]);
